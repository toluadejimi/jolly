...
class ProductTypeController extends Controller {
    ...
    public function downloadProductTypes(Request $request) {
        $productTypes = ProductType::all();
        $filename = 'product_types_' . time();
        $columns = ['id', 'name'];
        $headings = ['ID', 'Name'];

        $downloader = new Downloader($filename, $productTypes, $columns, $headings);

        if ($request->type == 'pdf') {
            return $downloader->downloadAsPdf('Product Types');
        } else {
            return $downloader->downloadAsCsv($productTypes);
        }
    }

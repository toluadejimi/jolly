...
class ProductValidationService
{
    ...
    public function validationRules($request, $product = null)
    {
        ...
        return [
            ...
            // Media Contents
            'main_image_id'             => 'nullable|numeric|exists:media,id',
     ...
     *
     * @param $request The request containing digital product file data.
     * @param $product The product being validated.
     *
     * @return array The validation rules for the digital product file.
     */
    private function digitalProductFileValidationRule($request, $product)
    {
        $rule = 'nullable';
        ...
        if ($request->is_downloadable == Status::YES && $request->product_type == Status::PRODUCT_TYPE_SIMPLE && $request->delivery_type == Status::DOWNLOAD_INSTANT) {
            $rule = $product && $product->id && !$product->digitalFile ? 'required' : 'nullable';
        }

<div class="xzoom-container product-gallery" style="position: relative;">
    ...
    <img class="@if ($product->is_downloadable) downloadable_image @else xzoom5 @endif"
         id="xzoom-magnific"
         src="{{ $images->first()?->full_url ?? asset('assets/images/default.png') }}"
         xoriginal="{{ $images->first()?->full_url }}" />
        ...
        <div class="quick-view-slider owl-carousel owl-theme">
            @foreach ($images as $image)
                <a href="{{ $image->full_url }}" class="media-item-nav" data-media_id="{{ $image->id }}">
                    <img class="@if ($product->is_downloadable) downloadable_gallery @else xzoom-gallery5 @endif"
                         width="80"
                         src="{{ getImage($image->path . '/thumb_' . $image->file_name) }}"
                         xpreview="{{ $image->full_url }}">
                </a>
            @endforeach
        </div>
    </div>

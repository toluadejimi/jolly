  ...
  <div class="row">
    <div class="col sidebar mb-3">
      ...
      <div class="list-group div-scroll">
        ...
        @endforeach
        @php
          $folderSuffix = ($current_folder) ? '&f=' . \Illuminate\Support\Facades\Crypt::encrypt($current_folder) : '';
        @endphp
        @foreach($files as $file)
          <div class="list-group-item">
            <div class="log-file-row">
              <a href="?l={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}{{ $folderSuffix }}"
                 class="log-file-name @if ($current_file == $file) llv-active @endif">
                {{ $file }}
              </a>
              <span class="log-file-actions">
                <a href="?clean={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}{{ $folderSuffix }}" class="llv-clean" title="Clear log">Clear</a>
                <a href="?del={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}{{ $folderSuffix }}" class="llv-del" title="Delete file">Delete</a>
              </span>
            </div>
          </div>
        @endforeach
        @if(count($files) > 0)
          <div class="list-group-item">
            <a id="delete-all-sidebar" href="?delall=true{{ $folderSuffix }}" title="Delete all log files in this channel">
...
<script>
  ...
  $(document).ready(function () {
    ...
    $('#delete-log, #clean-log, #delete-all-log, #delete-all-sidebar, .llv-clean, .llv-del').on('click', function () {
